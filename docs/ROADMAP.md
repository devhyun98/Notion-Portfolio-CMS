...
- **`/projects` 검색/필터**
  - ✅ 제목 + 설명 텍스트 검색
  - ✅ 태그별 필터 사이드바
  - ✅ 필터 상태를 URL 쿼리 파라미터로 저장 (`?search=...&tags=...`)
  - ✅ 모바일용 FilterSheet 컴포넌트 추가
...
- 컴포넌트: `<SearchBox>`, `<TagFilter>`, `<FilterSheet>` ✅
- 훅: `use-search.ts`, `use-filters.ts`, `use-filter-sync.ts` ✅
...
- **`/about`**
  ...
  - ✅ 기술 스택 (카테고리별)
  - ✅ 연락처 정보
  - ✅ about-data.ts로 정적 데이터 중앙화

#### 4-3. 관련 콘텐츠 섹션 ✅ 완료
- ✅ 프로젝트/블로그 상세 페이지 하단에 같은 태그의 다른 항목 3-4개 표시
- ✅ `<RelatedProjects>`, `<RelatedBlogs>` 컴포넌트 타입 개선
- ✅ 공통 태그 기반 정렬

#### 4-4. 태그 페이지 ✅ 완료
- **`/tags/[tag]`**
  - ✅ 특정 태그의 모든 프로젝트 + 블로그 글 통합 표시
  - ✅ 동적 파라미터 생성 (generateStaticParams)
  - ✅ 태그별 아카이브 페이지
...
- **`/resume`**
  - ✅ 이력서 HTML 표시
  - ✅ 경력, 기술스택, 교육, 자격증 정보
  - ✅ PDF 다운로드 링크 (UI 준비, 기능은 선택사항)
  - ✅ 네비게이션 추가

#### 4-6. 블로그 목차 (Table of Contents) ✅ 완료
- ✅ 마크다운 헤딩 기반 자동 생성
- ✅ 사이드바에서 sticky 위치 설정
- ✅ 클릭하면 해당 섹션으로 스크롤
- ✅ 반응형 레이아웃 (lg 화면에서만 표시)
...
### 완료 기준
- [x] 검색 기능이 정확한 결과 반환
- [x] 필터 적용 시 URL이 변경되고, URL 직접 접근 시 필터 상태 복원
- [x] About 페이지에 모든 정보 표시
- [x] 관련 콘텐츠 추천이 올바르게 작동
- [x] 목차가 자동으로 생성되고 스크롤 추적
- [x] 모든 새 페이지가 반응형으로 동작
- [x] `npm run build` 성공
...
**2026-06-04 구현된 기능:**
- [x] 태그 페이지 (`/tags/[tag]`) - 태그별 프로젝트와 블로그 글 통합 표시
- [x] 이력서 페이지 (`/resume`) - 경력, 기술스택, 교육, 자격증
- [x] FilterSheet 컴포넌트 - 모바일용 필터 UI
- [x] About 페이지 개선 - about-data.ts로 정적 데이터 관리
- [x] 검색/필터 개선 - URL 양방향 동기화
- [x] 네비게이션 업데이트 - 이력서 링크 추가
- [x] TagBadge 개선 - 태그 페이지로의 링크 생성
- [x] 관련 콘텐츠 타입 개선 - RelatedContent 타입 안정성 강화
- [x] 전체 빌드 성공 및 TypeScript 검증 완료 ✅

**새로운 파일:**
- `src/app/(marketing)/tags/[tag]/page.tsx` - 태그별 아카이브 페이지
- `src/app/(marketing)/resume/page.tsx` - 이력서 페이지
