...
### 완료 기준
- [ ] Notion API 클라이언트가 정상 작동 (테스트 콘솔로 데이터 확인)
- [ ] 모든 타입 정의 완료
- [ ] ProjectCard, BlogCard 컴포넌트 Storybook 또는 개별 테스트
- [ ] 마크다운 렌더링 테스트 완료 (코드 블록, 제목 등)
- [ ] 유틸리티 함수 단위 테스트 (날짜, slug 생성 등)
...
### 완료 기준
- [ ] 모든 페이지에서 Notion 데이터가 정상 표시됨
- [ ] 마크다운 콘텐츠가 올바르게 렌더링됨
- [ ] 모바일/태블릿/데스크톱에서 반응형으로 동작
- [ ] `npm run build` 성공
- [ ] Lighthouse 성능 점수 70점 이상
- [ ] 모든 링크가 정상 작동 (이전/다음 네비게이션 포함)
...
| Phase | 단계 | 예상 기간 | 상태 |
|-------|------|---------|------|
| **Phase 1** | 프로젝트 초기 설정 | 2-3시간 | ✅ 완료 |
| **Phase 2** | 공통 모듈/컴포넌트 | 3-4일 | ⏳ 대기 중 |
| **Phase 3** | 핵심 기능 (페이지) | 7-10일 | ⏳ 대기 중 |
| **Phase 4** | 추가 기능 | 5-7일 | ⏳ 대기 중 |
| **Phase 5** | 최적화 및 배포 | 6-8일 | ⏳ 대기 중 |
| **총 소요 시간** | - | **약 3-4주** | - |
...
1. **Phase 2 시작**: Notion API 클라이언트 구현부터 시작
2. **의존성 추가**: 마크다운 렌더러 (remark, rehype) 등 필수 라이브러리 설치
3. **테스트**: 각 단계가 끝날 때마다 `npm run build` 및 Lighthouse 검사
4. **배포 준비**: Phase 4 완료 후 Vercel 배포 계획
