...
| **Phase 2** | 공통 모듈/컴포넌트 | 3-4일 | ✅ 완료 |
| **Phase 3** | 핵심 기능 (페이지) | 7-10일 | ⏳ 진행 중 |
...
1. **Phase 3 시작**: 핵심 페이지 구현 (프로젝트, 블로그, 홈 페이지)
   - 3-1: 프로젝트 페이지 (`/projects`, `/projects/[slug]`)
   - 3-2: 블로그 페이지 (`/blog`, `/blog/[slug]`)
   - 3-3: 홈 페이지 (`/`)
   - 3-4: 헤더/푸터 레이아웃
2. **테스트**: 각 단계가 끝날 때마다 `npm run build` 및 반응형 테스트
3. **Phase 4 진행**: 검색, 필터, About 페이지 등 추가 기능
4. **배포 준비**: Phase 5 최적화 및 Vercel 배포
